...
const I18N = {
    zh: {
        ...
        example_business_title: '高新技术企业申报', example_business_text: '请开始高企申报',
    ...
    en: {
        ...
        example_business_title: 'Business', example_business_text: 'Start enterprise declaration process',
...
function startSSE(requestId, loadingEl, timestamp) {
    // 使用Last-Event-ID机制支持断点续传
    let lastEventId = 0;
    let es = null;
    
    function createEventSource() {
        const url = `/stream?request_id=${encodeURIComponent(requestId)}&last_event_id=${lastEventId}`;
        es = new EventSource(url);
        activeStreams[requestId] = es;
        
        // 设置重连时间（服务器可以通过retry字段控制）
        es.onopen = function() {
            console.log(`SSE连接已建立，Last-Event-ID: ${lastEventId}`);
        };
        
        es.onerror = function(e) {
            console.error('SSE连接错误:', e);
            // EventSource会自动重连，我们只需要记录错误
            if (es.readyState === EventSource.CLOSED) {
                console.log('SSE连接已关闭，等待自动重连...');
            }
        };
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-4xl">
            ...
            <div class="example-card group bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-white/10 rounded-xl p-4 cursor-pointer hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-md transition-all duration-200">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="font-medium text-sm text-slate-700 dark:text-slate-200" data-i18n="example_business_title">${t('example_business_title')}</span>
                </div>
                <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed" data-i18n="example_business_text">${t('example_business_text')}</p>
            </div>
        </div>
    `;
...
function handleDialogFileSelection(event) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // 检查文件类型
        const allowedTypes = ['.pdf', '.xlsx', '.xls', '.docx', '.doc', '.txt', '.jpg', '.jpeg', '.png', '.zip', '.rar', '.7z', '.tar', '.gz'];
